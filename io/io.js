...
module.exports = function (server) {
    ...
    io.on("connection", socket => {
            ...
            if (data.id) {
                try {
                    if (! users[data.id].lobby) {
                        ...
                        lobbies[id].emit("new_entry", {msg : "You entered the lobby", index : lobbies[id].users.length -1, id : data.id})
                        cb("", {"code" : id});
                    }
                    else {
                ...
                try {
                    if (! users[data.id].lobby) {
                        ...
                        if (data.code && lobbies[data.code]) {
                            if (lobbies[data.code].enter(data.id, data.name)) {
                                cb("", succ['joined_lobby']);
                                users[data.id].lobby = lobbies[data.code];
                                lobbies[data.code].emit("new_entry", {msg : data.name + " joined", index : lobbies[data.code].users.length -1, id : data.id})
                            }
                            else {
        ...
        socket.on("get_names", (data, cb) => {
            if (data.id) {
                if (users[data.id] && users[data.id].lobby) {
                    cb("", users[data.id].lobby);
                }
                else {
                    cb("not_a_member", err["not_a_member"]);
                }
